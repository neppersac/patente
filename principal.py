...
# Função para análise ETS com Walk-Forward
def melhor_ets_walk_forward(serie, initial_train_size, step_size, modelos, anos_previsao, titulo):
    rmse_list = []
    previsoes_futuras = []
    melhor_modelo_final = None
    menor_aic_final = float('inf')

    n = len(serie)
    for i in range(initial_train_size, n, step_size):
        treino = serie[:i]
        teste = serie[i:i+step_size] if i+step_size <= n else serie[i:]

        melhor_modelo, _ = melhor_ets(serie, treino, teste, modelos, f"{titulo} - Janela {i}")
        
        if melhor_modelo and melhor_modelo.aic < menor_aic_final:
            menor_aic_final = melhor_modelo.aic
            melhor_modelo_final = melhor_modelo
        
        previsoes = melhor_modelo.forecast(steps=len(teste))
        rmse = np.sqrt(mean_squared_error(teste, previsoes))
        rmse_list.append(rmse)

    # Previsão futura com o melhor modelo
    if melhor_modelo_final:
        previsao_futura = melhor_modelo_final.forecast(steps=anos_previsao)
        previsoes_futuras = previsao_futura

    return melhor_modelo_final, previsoes_futuras, np.mean(rmse_list)
...
# Função para análise ARIMA com Walk-Forward
def analisar_arima_walk_forward(serie, initial_train_size, step_size, anos_previsao):
    ...
    for i in range(initial_train_size, n, step_size):
        ...
        try:
            ...
            model_fit = model.fit()
            previsoes = model_fit.forecast(steps=len(teste))
            rmse = np.sqrt(mean_squared_error(teste, previsoes))
            rmse_list.append(rmse)
            
            if i + step_size >= n:  # Última iteração, faz previsão futura
                forecast = model_fit.get_forecast(steps=anos_previsao)
                previsoes_futuras = forecast.predicted_mean
...
# Divisão treino/teste ou Walk-Forward
serie = df[setor].values
n = len(serie)
initial_train_size = int(n * 0.6)  # Para Walk-Forward, começamos com 60% dos dados
step_size = 1  # Passo de 1 ano para Walk-Forward
...
with col_arima:
    ...
    try:
        if teste_metodo == 'Divisão 80/20':
            ...
            forecast_mean, conf_int, rmse_arima = analisar_arima_walk_forward(serie, initial_train_size, step_size, anos_previsao)
        
        ultimo_ano = df['ano'].iloc[-1]
        anos_futuros = list(range(ultimo_ano + 1, ultimo_ano + 1 + anos_previsao))
        
        fig, ax = plt.subplots(figsize=(8, 5))
        ax.plot(df['ano'], serie, label='Dados completos')
        ...
        ax.set_title(f'Previsão ARIMA - {setor.upper()} ({teste_metodo})')
        ax.set_xlabel('Ano')
...
with col_ets:
    ...
    try:
        if teste_metodo == 'Divisão 80/20':
            ...
            melhor_modelo, previsao_futura, rmse_ets = melhor_ets_walk_forward(serie, initial_train_size, step_size, modelos, anos_previsao, setor)
        ...
        if melhor_modelo:
            ...
            ax.set_title(f'Previsão ETS - {setor.upper()} ({teste_metodo})')
            ax.set_xlabel('Ano')
            ax.set_ylabel('Emissões CO2e')
            ax.grid(True)
            ax.legend()
